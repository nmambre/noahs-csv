...
# Read the CSV file
df_customers <- read.csv("data/5784-speedrun/noahs-customers.csv")
...
df_orders_items <- read.csv("data/5784-speedrun/noahs-orders_items.csv")
...
df_orders <- read.csv("data/5784-speedrun/noahs-orders.csv")
...
# get customers from df_customers where df_customers$customerid is in customer_ids and df_customers$initials is "DS"
customers_with_initials_DS <- df_customers[df_customers$customerid %in% customer_ids & df_customers$initials == "DS", ]
customers_with_initials_DS

# DAY 3: The Neighbor
# Create a vector of Goat years
goat_years <- c(1919,1931,1943,1955,1967,1979,1991,2003,2015)

# Initialize an empty data frame to store the final results
final_range <- data.frame()

# Loop through Goat years
for (year_goat in goat_years) {
  # Define the date range for Libra
  libra_start_date <- as.Date(paste(year_goat, "-09-23", sep = ""))
  libra_end_date <- as.Date(paste(year_goat, "-10-22", sep = ""))
  
  # Filter for individuals born between September 23 and October 22 in a specific Goat year
  libra_range <- df_customers[df_customers$birthdate >= libra_start_date & df_customers$birthdate <= libra_end_date, ]
  
  # Filter for individuals born in the specific Goat year
  goat_range <- df_customers[year(df_customers$birthdate) %% 12 == (year_goat %% 12), ]
  
  # Find the intersection of the two ranges
  intersection_range <- intersect(libra_range$customerid, goat_range$customerid)
  
  # Filter the original dataframe based on the intersection and append to final_range_goat
  final_range <- rbind(final_range, df_customers[df_customers$customerid %in% intersection_range, ])
}

# View the resulting data frame for Goat and Libra
#print(final_range)
...
df_products <- read.csv("data/5784-speedrun/noahs-products.csv")
...
#customers_cat <- df_customers[df_customers$customerid %in% customer_ids_cat_count$customerid &   substr(df_customers$citystatezip, 1, 8) == "Brooklyn", ]
customers_cat <- df_customers[df_customers$customerid %in% customer_ids_cat_count$customerid, ]
#customers_cat
...
bargain_hunter_products_col <- bargain_hunter_products[substr(bargain_hunter_products$sku, 1, 3) == "COL" | substr(bargain_hunter_products$sku, 1, 3) == "HOM", ]
bargain_hunter_products_col

# get all rows of bargain_hunter_order_items where sku starts with "COL"
bargain_hunter_order_items_col <- bargain_hunter_order_items[substr(bargain_hunter_order_items$sku, 1, 3) == "COL" | substr(bargain_hunter_products$sku, 1, 3) == "HOM", ]
...
same_shipped_order_items_col <- same_shipped_order_items[substr(same_shipped_order_items$sku, 1, 3) == "COL" | substr(bargain_hunter_products$sku, 1, 3) == "HOM", ]
...
df_products_col <- df_products[substr(df_products$sku, 1, 3) == "COL" | substr(df_products$sku, 1, 3) == "HOM", ]
nrow(df_products_col)

# get all rows from df_orders_items where df_orders_items$sku starts with "COL"
df_orders_items_col <- df_orders_items[substr(df_orders_items$sku, 1, 3) == "COL" | substr(df_orders_items$sku, 1, 3) == "HOM", ]
